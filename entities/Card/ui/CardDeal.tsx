...
import { CardPhone } from "../type/model";
import { DEAL_STATUS_COLORS, DEAL_STATUS_LABELS } from "../type/status";

export function CardDeal({ brand, model, imageUrl, price, oldPrice, id, variants, status}: CardPhone) {
 ...
 const handleClick = () => {
        addToCart({id: String(id), model, brand, colorId:firstColor.name, capcity: firstVariant?.capacity, price, quantity: 1, imageUrl});
      }
    return (
        <article
                    key={id}
                    ...
                    <div className="w-full max-w-56">
                        ...
                        <div className="mt-3 mb-2 flex flex-col gap-1">
                            ...
                            <div className="w-full h-1.5 rounded-full bg-[#E5E7EB] relative">
                                <span className={`absolute top-0 left-0 h-full bg-[#4F46E5] rounded-full `} style={{ width: `${stock && ((stock / 50) * 100)}%` }}></span>
                            </div>
                        </div>
