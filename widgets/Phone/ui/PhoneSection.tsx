...
export function PhoneSection() {
  ...
  if (!product) return <PhoneNotFound />;
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      // Use the selected color if the user picked one, otherwise fall back to first.
      colorId: selectedColor || firstColor?.name || "",
    ...
    <main className="px-4 py-8">
      <Container>
        <div className="grid lg:grid-cols-2 gap-12">
          {/* Product image */}
          <div className="bg-linear-to-br from-gray-100 to-gray-50 rounded-3xl flex items-center justify-center min-h-100 relative">
            <Image
              src={product.imageUrl}
              alt={product.fullName ?? product.model}
              width={400}
              height={400}
          ...
          <div>
            ...
            <p className="text-gray-500 mb-4">
              {firstColor?.name} · {firstVariant?.capacity}
            </p>
            ...
            {/* Key features — typed as string[], no `any` needed */}
            <div className="mb-6">
