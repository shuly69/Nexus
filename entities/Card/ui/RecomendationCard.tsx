...
export function RecommendationCard({ brand, model, id, color, capacity, price, oldPrice, imageUrl, slug, specs, description, features }: { brand: string; model: string; id: string; color: string; capacity: string; price: number; oldPrice?: number; imageUrl: string , slug: string | undefined; specs: ProductSpecs; description: string; features: string[] }) {
    return (
        <article className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 flex flex-col items-center w-50 min-h-70">
            ...
            <div className="flex justify-between w-full mt-4 ">
                ...
                <AddToCartButton id={id} model={model} brand={brand} color={color} capacity={capacity} price={price} imageUrl={imageUrl} specs={specs} description={description} features={features} />
            </div>
